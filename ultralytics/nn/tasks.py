        ...
        for m in self.model:
            if m.f != -1:  # if not from previous layer
                ...
                if isinstance(m.f, list):
                    # 处理多输入模块
                    x_list = [x if j == -1 else y[j] for j in m.f]

                    # Modules marked with ``accepts_multiple_inputs`` receive
                    # their feature maps as separate positional arguments.
                    if getattr(m, "accepts_multiple_inputs", False):
                        if len(x_list) == 2:
                            x = m(x_list[0], x_list[1])
                        else:
                            raise ValueError(f"{m.type} expects 2 inputs, got {len(x_list)}")
                    ...
                    # 单输入模块
                    x = y[m.f] if isinstance(m.f, int) else x
                    already_forwarded = False
                # =======================================
            else:
                already_forwarded = False
            ...
            # Custom multi-input modules have already executed their forward.
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # Multi-input feature fusion modules receive channel sizes from the
        # referenced layers. ``auto`` keeps the config valid across model
        # scales while explicit values remain supported for reproducibility.
        if m is CSCG:
            if isinstance(f, list) and len(f) == 2:
                low_channels, high_channels = ch[f[0]], ch[f[1]]
                if len(args) < 4:
                    raise ValueError(
                        f"{m.__name__} requires [low_ch, high_ch, out_ch, scale], got {args}"
                    )
                low_ch, high_ch, out_ch, scale_factor = args[:4]
                low_ch = low_channels if low_ch == "auto" else int(low_ch)
                high_ch = high_channels if high_ch == "auto" else int(high_ch)
                out_ch = high_channels if out_ch == "auto" else int(out_ch)
                args = [low_ch, high_ch, out_ch, scale_factor] + args[4:]
                c2 = out_ch
            else:
                raise ValueError(f"{m.__name__} requires exactly 2 input layers in 'from', got {f}")
